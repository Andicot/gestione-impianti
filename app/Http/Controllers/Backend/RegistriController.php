...
class RegistriController extends Controller
{
    ...
    protected function registroLogin($request)
    {
        ...
        if (\Auth::id() > 1) {
            $recordsQB->where('user_id', '>', 1)->where(function ($query) use ($request) {
                $query->where('impersonato_da_id', '>', 1)->orWhereNull('impersonato_da_id');
            });
        }
        ...
        return view('Backend.Registri.indexLogin' . ucfirst(env('CRUD_TEMPLATE')))->with([
            'records' => $records,
            'filtro' => $filtro,
    ...
    {
        $recordsQB = Audit::with('user:id,cognome,nome')->with('auditable');
