    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ \App\storeUpdateHelper($controller, $record?->id) }}" onsubmit="return disableSubmitButton()">
                ...
                {{-- Sezione Dispositivi Multipli (solo se tecnico) --}}
                <div class="mt-6" id="sezione-dispositivi-multipli" style="display: none;">
                    <div class="separator separator-content mb-6 flex-grow-1 me-3">
                        <h4 class="w-300px">Dispositivi Coinvolti</h4>
                    </div>

                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label class="form-label">Seleziona più dispositivi (opzionale)</label>
                                <select name="dispositivi_coinvolti[]" id="dispositivi_coinvolti" class="form-select" multiple>
                                    @foreach($dispositivi as $dispositivo)
                                        <option value="{{ $dispositivo->id }}"
                                                @if(in_array($dispositivo->id, old('dispositivi_coinvolti', $record->dispositivi_coinvolti ?? []))) selected @endif>
                                            {{ $dispositivo->matricola }} - {{ ucfirst($dispositivo->tipo) }}
                                            @if($dispositivo->unitaImmobiliare)
                                                ({{ $dispositivo->unitaImmobiliare->scala }} - {{ $dispositivo->unitaImmobiliare->interno }})
                                            @endif
                                        </option>
                                    @endforeach
                                </select>
                                <div class="form-text">Seleziona se il problema riguarda più dispositivi</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="alert alert-primary d-flex align-items-center p-5 mt-10">
                        <span class="svg-icon svg-icon-2hx svg-icon-primary me-4">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                <path opacity="0.3" d="M20.5543 4.37824L12.1798 2.02473C12.0626 1.99176 11.9376 1.99176 11.8203 2.02473L3.44572 4.37824C3.18118 4.45258 3 4.6807 3 4.93945V13.569C3 14.6914 3.48509 15.8404 4.4417 16.984C5.17231 17.8575 6.18314 18.7345 7.446 19.5909C9.56752 21.0295 11.6566 21.912 11.7445 21.9488C11.8258 21.9829 11.9129 22 12.0001 22C12.0872 22 12.1744 21.9829 12.2557 21.9488C12.3435 21.912 14.4326 21.0295 16.5541 19.5909C17.8169 18.7345 18.8277 17.8575 19.5584 16.984C20.515 15.8404 21 14.6914 21 13.569V4.93945C21 4.6807 20.8189 4.45258 20.5543 4.37824Z" fill="#000000"/>
                                <path d="M10.5606 11.3042L9.57283 10.3018C9.28174 10.0065 8.80522 10.0065 8.51412 10.3018C8.22897 10.5912 8.22897 11.0559 8.51412 11.3452L10.4182 13.2773C10.8099 13.6747 11.451 13.6747 11.8427 13.2773L15.4859 9.58051C15.771 9.29117 15.771 8.82648 15.4859 8.53714C15.1948 8.24176 14.7183 8.24176 14.4272 8.53714L11.7002 11.3042C11.3869 11.6221 10.874 11.6221 10.5606 11.3042Z" fill="#000000"/>
                            </svg>
                        </span>
                ...
                @else
                    ...
                    <div class="alert alert-success d-flex align-items-center p-5 mt-10">
                        <span class="svg-icon svg-icon-2hx svg-icon-success me-4">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                <path opacity="0.3" d="M20.5543 4.37824L12.1798 2.02473C12.0626 1.99176 11.9376 1.99176 11.8203 2.02473L3.44572 4.37824C3.18118 4.45258 3 4.6807 3 4.93945V13.569C3 14.6914 3.48509 15.8404 4.4417 16.984C5.17231 17.8575 6.18314 18.7345 7.446 19.5909C9.56752 21.0295 11.6566 21.912 11.7445 21.9488C11.8258 21.9829 11.9129 22 12.0001 22C12.0872 22 12.1744 21.9829 12.2557 21.9488C12.3435 21.912 14.4326 21.0295 16.5541 19.5909C17.8169 18.7345 18.8277 17.8575 19.5584 16.984C20.515 15.8404 21 14.6914 21 13.569V4.93945C21 4.6807 20.8189 4.45258 20.5543 4.37824Z" fill="#000000"/>
                                <path d="M14.8563 9.1903C15.0606 8.94984 15.0606 8.65016 14.8563 8.4097C14.6521 8.16924 14.3871 8.16924 14.1828 8.4097L12.4828 10.297L11.4828 9.1903C11.2785 8.94984 11.0135 8.94984 10.8092 9.1903C10.6049 9.43076 10.6049 9.73044 10.8092 9.9709L12.1425 11.4709C12.3468 11.7114 12.6118 11.7114 12.8161 11.4709L14.8563 9.1903Z" fill="#000000"/>
                            </svg>
                        </span>
                        <div class="d-flex flex-stack flex-grow-1 flex-wrap flex-md-nowrap">
                            <div class="mb-3 mb-md-0 fw-bold">
...
@push('customScript')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Gestione categoria per mostrare/nascondere sezione dispositivi multipli
            $('#categoria').change(function() {
                const categorieTecniche = ['errore_dispositivo', 'comunicazione_concentratore', 'manutenzione', 'tecnico'];
                const categoria = $(this).val();

                if (categorieTecniche.includes(categoria)) {
                    $('#sezione-dispositivi-multipli').show();
                } else {
                    $('#sezione-dispositivi-multipli').hide();
                    $('#dispositivi_coinvolti').val([]).trigger('change');
                }
            });
            ...
            // Inizializza select multiplo per dispositivi
            $('#dispositivi_coinvolti').select2({
